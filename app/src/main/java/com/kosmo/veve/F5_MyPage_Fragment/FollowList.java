...
public class FollowList extends AppCompatActivity implements Runnable{
    ...
    ArrayList<GallaryFile> follow_file_list = new ArrayList<>();
    ...
    private void initAdapter() {
        recyclerViewAdapter = new FollowList_RecyclerViewAdapter(follow_list,follow_file_list);
    ...
    public void run() {
        try {
            ...
            for (int i = 0; i < jArray.length(); i++) {
                ...
                gf.setUserID(row.getString("userID"));
                gf.setF_name(row.getString("f_name"));
                follow_list.add(gb);
                follow_file_list.add(gf);
            }
